...
class human:
    ...
    def validate_DOFs(self):
        ...
        for i in np.arange(len(self.DOF)):
            if (self.DOF[human.DOFnames[i]] < human.DOFbounds[i][0] or 
                self.DOF[human.DOFnames[i]] > human.DOFbounds[i][1]):
                print "Joint angle",human.DOFnames[i],"=",\
                      self.DOF[human.DOFnames[i]]/np.pi,\
                      "pi-rad is out of range. Must be between",\
                      human.DOFbounds[i][0]/np.pi,"and",\
                      human.DOFbounds[i][1]/np.pi,"pi-rad"
                boolval = -1
        return boolval
    ...
    def define_segments(self):
        ...
        CRotMat = self.s[2].RotMat * mymath.Rotate3(
                                    [0,
                                     self.DOF['TClateralSpinalFlexion'],
                                     self.DOF['TCspinalTorsion']])
